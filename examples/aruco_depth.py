...
class ArucoDetector:
		...
		if ids is not None:
			...
			for j in range(len(corners)):
				...
				for i in range(corners[j].shape[1]):
					...
					c2 = calibration.convert_2d_to_2d(pixels, depth_image[pix_y, pix_x], K4A_CALIBRATION_TYPE_COLOR, K4A_CALIBRATION_TYPE_DEPTH)
	...
	def detect(self, calibration, color_image, depth_image):
		...
		c0, x_vec_norm, y_vec_norm, z_vec_norm, corner_dict_2d, corner_dict_3d = self.get_reference_frame(calibration, ids, corners, depth_image)
		# image = self.draw_aruco_box(image, ids, corners, corner_dict_2d)
